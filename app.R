...
ui <- dashboardPage(skin = "green",
                    ...
                    dashboardSidebar(
                      ...
                      checkboxGroupInput("genders", "Gender",
                                         ...
                                         selected = c("male", "female", "non-binary"),width=300),
                      checkboxGroupInput("size", "Select Agency Size",
                                       choices = list("Small" = "Small",
                                                      "Medium" = "Medium",
                                                      "Big" = "Big"),
                                       selected = c("Small","Medium","Big"),width=300)),
                    dashboardBody( min_width=950,
                                   tabBox(id="tabs", type = "tabs", width = 12,
                                          selectInput("questions","Select Choice",
                                                      choices = employee_quest,
                                                      selected = NULL,
                                                      multiple = FALSE,
                                                      selectize = TRUE,
                                                      width = NULL,
                                          ...
                                          tabPanel("Age Distribution", tags$p("Select Survey Questions to see Age distribution"), hr(),
                                                   plotOutput("AgePlot", 
                                                                height= "790px", inline=FALSE )),
              
                                          tabPanel("For Employees", tags$p("Select Survey Questions"), hr(),
...
server <- function(input, output) {
  ...
  data_filtered <- reactive({results%>%
    filter(Age >= input$AgeInput[1],
           Age <= input$AgeInput[2])%>%
    filter(Gender %in% input$genders)%>%
    filter(Agency_size %in% input$size)})
    
  
  
  output$AgePlot <- renderPlot(
    ggplot(data_filtered(), aes(Age),fill=input$questions)+
      geom_histogram()
  )
}
